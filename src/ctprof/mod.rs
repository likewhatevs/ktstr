...
pub struct ThreadState {
    ...
    /// so the gating is an in-VM no-op; it only changes host-facing `ctprof
    /// capture` against single-family kernels.
    pub taskstats_measured: bool,
    /// Host-wide enablement of the `cpu_delay_*` sub-family (sched_info-sourced,
    ...
    /// Host-wide enablement of the delayacct resource-wait sub-family (`blkio` /
    ...
    /// Host-wide enablement of the xacct watermark sub-family
...
impl ThreadState {
    ...
    pub(crate) fn apply_delay_stats(
        &mut self,
        ds: &crate::taskstats::DelayStats,
        active: crate::host_context::TaskstatsActive,
    ) {
        ...
        // real taskstats payload: mark the payload measured (query Ok) so the
        // group aggregation (`ctprof_compare::groups::measured_predicate`) can
        // distinguish a measured zero from a never-captured payload
        // (CONFIG_TASKSTATS off / no CAP_NET_ADMIN / query raced exit), which
        // otherwise both read as 0. The per-sub-family enablement (host-global,
        // probed once per snapshot) is baked in alongside; the predicate ANDs the
        // query-Ok flag with the matching sub-family flag, so a sub-family
        // disabled while the query succeeds renders "-" not "0" (see the field
        // docs).
        self.taskstats_measured = true;
        self.cpu_delay_active = active.cpu_delay;
        self.delay_block_active = active.delay_block;
        self.xacct_active = active.xacct;
    }
...
/// [`capture_with`] with an injected phase-1 pool builder.
///
/// Production supplies Rayon's bounded [`rayon::ThreadPoolBuilder`].
/// Tests inject a deterministic build failure to exercise the
/// resource-exhaustion fallback without manipulating process limits.
fn capture_with_pool_builder<B>(
    proc_root: &Path,
    cgroup_root: &Path,
    sys_root: &Path,
    use_syscall_affinity: bool,
    ...
    C: FnOnce() -> usize,
{
        ...
        if use_syscall_affinity {
            ...
            let attach_one = |tgid: i32| {
                ...
                let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    let cache_key = std::fs::metadata(proc_root.join(tgid.to_string()).join("exe"))
                        .ok()
                        .map(|m| {
    ...
    // Per-sub-family enablement, probed once per snapshot from the host
    // (`/proc/sys/kernel/task_delayacct` + `/proc/config.gz`); baked into each
    // captured thread by `apply_delay_stats`, AND-ed with the per-thread query-Ok
    // in the group measured predicate. Only consumed on the live capture path.
    // Read independently of `collect_host_context`'s host-field probe (two small
    // /proc reads per infrequent snapshot) so the gating does not depend on
    // host-context collection running on this path.
    let taskstats_active = crate::host_context::probe_taskstats_active();
    ...
    for tgid in &tgids {
        ...
        for tid in iter_task_ids_at(proc_root, tgid) {
            ...
            if let Some(client) = taskstats_client.as_ref() {
                ...
                if let Ok(ds) = result {
                    t.apply_delay_stats(&ds, taskstats_active);
                }
            }
...
fn capture_pid_with(
    ...
    // Per-sub-family enablement, probed once per snapshot (see `capture_with`).
    let taskstats_active = crate::host_context::probe_taskstats_active();
