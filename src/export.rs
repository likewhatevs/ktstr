...
fn generate_preamble(
    ...
    let sched_args_joined = compose_sched_args_joined(entry, config_additions);
    ...
    let scheduler_launch = scheduler_launch_block(has_scheduler, &sched_args_joined);

    let mut script = script_header_and_spec(
        &test_name,
        &scheduler_name,
        &git_hash,
        need_llcs,
    ...
    script.push_str(&script_arg_parser_and_help());
    script.push_str(&script_prereq_and_topology_checks());
    script.push_str(&script_dispatch(&scheduler_launch));
    script
}

/// Assemble the scheduler argv string interpolated into the
/// launch line: config-addition prefixes first, then the joined
/// base sched-args.
fn compose_sched_args_joined(
    entry: &KtstrTestEntry,
    config_additions: &[ConfigExportAddition],
) -> String {
...
/// Build the scheduler-launch shell block (launch + 10s attach-wait
/// loop) for a scheduler-bearing test, or the empty string for a
/// binary-kind / EEVDF payload.
fn scheduler_launch_block(has_scheduler: bool, sched_args_joined: &str) -> String {
    if has_scheduler {
        format!(
            r#"
# --- scheduler launch ---
...
/// Script segment 1: shebang, `set -euo pipefail`, and the frozen
/// test-specification variable block. Ends with the blank line
/// before the arg parser.
#[allow(clippy::too_many_arguments)]
fn script_header_and_spec(
    test_name: &str,
    scheduler_name: &str,
    git_hash: &str,
    need_llcs: u32,
...
/// Script segment 2: arg parser, `--help` heredoc, banner, and the
/// root check. Leads with the blank line after the spec block; ends
/// with the root-check `fi`. No interpolation vars — every value is
/// a runtime shell variable.
fn script_arg_parser_and_help() -> String {
    String::from(
        r#"
QUIET=0
DURATION_OVERRIDE=""
...
/// Script segment 3: prereq checks, sched_ext conflict check, and
/// topology check. Leads with the blank line after the root check;
/// ends with the blank line before the archive-extract block. No
/// interpolation vars — every value is a runtime shell variable.
fn script_prereq_and_topology_checks() -> String {
    String::from(
        r#"
# --- prereq checks ---
...
/// Script segment 4: archive extraction, the EXIT-trap cleanup,
/// base64 decode, the interpolated scheduler-launch block, and the
/// in-process test dispatch. Leads with the blank line after the
/// topology check. `{scheduler_launch}` is the only interpolation;
/// every other value is a runtime shell variable.
fn script_dispatch(scheduler_launch: &str) -> String {
    format!(
        r#"
