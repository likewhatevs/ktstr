...
/// background handle `wait`/`kill` paths. The [`crate::test_support::PayloadMetrics`] is
...
/// Emit a [`crate::test_support::PayloadMetrics`] on the guest-to-host bulk data channel
...
/// `SIGCHLD = SIG_IGN` at startup in `src/vmm/rust_init/init.rs`
...
///
/// The shell-exec mode in `src/vmm/rust_init/modes.rs` already documents
...
/// running because `std::process::Child`'s `Drop` is a no-op.
///
...
mod tests {
    ...
    /// Host-side decode of a guest-emitted [`crate::test_support::PayloadMetrics`] JSON
