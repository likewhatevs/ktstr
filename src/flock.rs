...
/// key off this constant so the `.locks/` convention is a single
/// source of truth — a future relocation updates exactly one
/// place. Also referenced by run-listing walkers' dotfile filter
/// ([`crate::test_support::sidecar::is_run_directory`]) to keep
/// the lock subdirectory out of "list runs" output.
...
/// on `target = "ktstr::flock"` (the `module_path!()` default
/// target) can see contention progress in real time without
/// rebuilding.
///
...
/// materialize the parent (e.g. via [`materialize`]) do not need
/// to gate the helper. Callers that want to fail fast on a
...
";
    ...
    // ---------------------------------------------------------------
    // acquire_flock_with_timeout — shared primitive
    // ---------------------------------------------------------------
    //
    // The two prior call sites (cache-entry locks, run-dir sidecar
    // locks) each had their own integration tests covering the same
    // contract through different wrappers. The tests below exercise
    // the shared helper directly so a regression that only affects
    // the generic primitive (and not either wrapper) surfaces here
    // first.
