...
| `--memory-mb MB` | auto | Guest memory in MiB (minimum 128). The CLI flag name uses the `mb` shorthand for backward compatibility with existing scripts, but the value is interpreted as binary mebibytes — `--memory-mb 2048` allocates `2048 << 20` = 2 GiB. When absent, estimated from payload and include file sizes. |
