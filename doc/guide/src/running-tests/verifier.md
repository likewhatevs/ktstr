...
3. **Verifier collection** — inside the VM, the scheduler
   ...
   the virtio-console bulk port between
...
cell-emission time. The `SKIP`-banner/exit-0 arm for such a cell is
defense-in-depth that only fires when the cell reaches the handler
with a matching kernel label present in `KTSTR_KERNEL_LIST` (i.e.
under the `cargo ktstr verifier` dispatcher's env). A bare
`--exact verifier/...` run with no `KTSTR_KERNEL_LIST` instead errors
with exit 1 (the empty-kernel-list diagnostic, checked before the SKIP
arm).
