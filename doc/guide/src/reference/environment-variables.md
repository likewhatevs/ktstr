...
| `KTSTR_CI` | Set to any non-empty value to flip every sidecar's `run_source` field from `"local"` (developer-machine default) to `"ci"`. Read at sidecar-write time by `detect_run_source`; surfaces through the `run_source` filter (visible via `cargo ktstr stats list-values`) so CI-produced runs can be distinguished from developer runs without per-run directory bookkeeping. Empty string counts as unset. The third value `"archive"` is applied at LOAD time (not write time) when `cargo ktstr stats list-values --dir` pulls sidecars from a non-default pool root — `KTSTR_CI` does not control that. | None (`run_source = "local"`) |
...
| `KTSTR_CACHE_DIR` | Override ktstr's cache ROOT — every cache type (kernel images, BTF-anchor, blobs), not just kernels. When set, the path is used verbatim as the root (no per-type subdir like `kernels` is appended). | `$XDG_CACHE_HOME/ktstr/` or `$HOME/.cache/ktstr/` (per-type subdir appended, e.g. `kernels/`) |
...
| `KTSTR_SIDECAR_DIR` | Directory for per-test result sidecar JSON files. Used as-is when set, no key suffix. Consumed by the test harness (sidecar write path) and by bare `cargo ktstr stats` (sidecar read path). When this override is set, **pre-clear is skipped AND the per-run-key cross-process flock is skipped** — the operator chose the directory and owns its contents, so any pre-existing sidecars there are preserved, and ktstr does not coordinate concurrent writers against the override path. Two concurrent runs pointing the same `KTSTR_SIDECAR_DIR` at the same path therefore have no serialization between them; choose distinct override paths per process (or rely on the default-path branch, which acquires the flock automatically). `cargo ktstr stats list`, `cargo ktstr stats list-values`, and `cargo ktstr stats show-host` walk `{CARGO_TARGET_DIR or "target"}/ktstr/` by default and ignore `KTSTR_SIDECAR_DIR` — pass `--dir DIR` on `list-values` / `show-host` / `explain-sidecar` to point them at an alternate run root. See [Runs](../running-tests/runs.md). | `{run root}/{kernel}-{project_commit}/`, where **run root** is `KTSTR_RUNS_ROOT` when set (the `cargo ktstr` orchestrator stamps it to the absolute cargo target dir's `ktstr` subdir at startup, so it is the effective root under `cargo ktstr`), else `{CARGO_TARGET_DIR}/ktstr`, else `target/ktstr` (the raw `cargo nextest run` fallback). `{project_commit}` is the project HEAD short hex, suffixed `-dirty` when the worktree differs, or the literal `unknown` when not in a git repo — see [Runs](../running-tests/runs.md) for the unknown-commit collision semantics. This `CARGO_TARGET_DIR`/`target` formula is the raw-`cargo nextest run` fallback: under `cargo ktstr` the orchestrator stamps `KTSTR_RUNS_ROOT` once at startup and `runs_root()` reads it FIRST — before `CARGO_TARGET_DIR` — so child sidecar writes and the footer / `stats` / `replay` reads resolve the same absolute run root regardless of CWD. |
...
| `KTSTR_VERBOSE` | Set to `"1"` for verbose VM console output (`loglevel=7`, plus `earlyprintk=serial` on x86_64). | None |
...
| `RUST_LOG` | Controls every ktstr tracing filter — guest-side and host-side. **Guest-side**: propagated to the VM kernel command line and parsed by the guest tracing subscriber, so guest events are filtered by the same `RUST_LOG` value the host process saw at launch. **Host-side**: a useful target is `"ktstr::flock::acquire"` (the `module_path!()` default for `src/flock/acquire.rs`, where the shared flock-timeout primitive emits a `tracing::debug!("waiting on flock at …")` event on each poll iteration that loops back) — e.g. `RUST_LOG=ktstr::flock=debug` surfaces flock-contention heartbeats. Host-side tracing is emitted only when a subscriber is installed; operators wanting a sink (file, alternate format) install their own subscriber. | None |
...
| `KTSTR_DISK0_FS` / `KTSTR_DISK0_MOUNT` / `KTSTR_DISK0_RO` | Disk-attach metadata (fs type, mount point, ro flag) emitted by `src/vmm/setup/mod.rs` when `#[ktstr_test(disk = ...)]` is set and consumed by `src/vmm/rust_init/mounts.rs` to mount the virtio-blk backing inside the guest. |
