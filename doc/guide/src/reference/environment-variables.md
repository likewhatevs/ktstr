...
| `KTSTR_CONTENTION_BYPASS` | Set to any non-empty value to bypass the near-limit ENOMEM defensive check in the VMM. Operator escape hatch for environments where the host memory accounting is intentionally over-provisioned and the framework's contention guard would otherwise refuse to launch. | None (guard active) |
...
| `KTSTR_MODEL_OFFLINE` | Set to any non-empty value to disable the LLM model download path. The model layer returns its in-memory fallback summary instead of attempting a fetch. Useful in air-gapped CI or for fast iteration when model downloads are not required. | None (network fetch enabled) |
