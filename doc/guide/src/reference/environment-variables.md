...
| `KTSTR_CACHE_DIR` | Override ktstr's cache ROOT — every cache type (kernel images, BTF-anchor, blobs, LLM models), not just kernels. When set, the path is used verbatim as the root (no per-type subdir like `kernels`/`models` is appended). | `$XDG_CACHE_HOME/ktstr/` or `$HOME/.cache/ktstr/` (per-type subdir appended, e.g. `kernels/`, `models/`) |
